...
def train_and_save_color_perception_model(model_name, dataloaders, val_dataloader, criterion, optimizer, scheduler, stage_epochs, model_output_dir, loss_output_dir):
    ...
    for age in stage_epochs.keys():
        ...
        print(f"Training Color Perception for age: {age} months for {epochs_for_this_stage} epoch(s) on {model_name}")
        dataloader = dataloaders[age]
        stage_name = f"Color_Perception - Age {age} months - {model_name}"
        ...
        torch.save(model.state_dict(), os.path.join(model_output_dir, f"{model_name}_color_perception_model_age_{age}_months.pth"))
        ...
        # Save losses
        loss_file = f"{model_name}_color_perception_{age}_losses.json"
    ...
    final_stage_name = f"Color Perception Learning Curves - {model_name}"
...
def main():
    ...
    for model_name in model_names:
        ...
        train_and_save_color_perception_model(
            ...
            dataloaders=color_perception_dataloaders,
